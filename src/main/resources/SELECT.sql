...
/*
*Выбор столбцов id,first_name, last_name, всех объектов в таблице Employees  где department указан как kitchen
*/
...
/*
*Выбор столбцов id,first_name всех объектов в таблице Employees и  столбцов id,number,expire_date всех объектов в таблице Passports где  id из таблицы Passports соответствует значению позиции passport_id  в таблице Employees, причем из таблицы Employees берутся все объекты а из Passports только те которые имеют связь с Employees  по  passport_id
*/
...
Select e.id,e.first_name, p.id,p.number,p.expire_date from Employees e right join Passports p 
on p.id = e.passport_id where p.number >=1004;
